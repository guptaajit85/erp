...
class WorkOrderController extends Controller
{
    ...
    { 
		...
		$query = WorkOrder::where('status', '=', '1')->with('WorkOrderItem')->with('WorkOrderItemDetail')->with('GatePass')->orderByDesc('work_order_id');
    ...
    public function store(Request $request)
    {
		...
		if($is_saved)
		{
			foreach($chk_sale_order_item_id_Arr as $soId)
			{
				...
				$woidArr->customer_id  						= $customerId;
				$woidArr->sale_order_id  					= $soItem->sale_order_id;
				$woidArr->sale_order_item_id  				= $soItem->sale_order_item_id;
				$woidArr->item_type_id  					= $soItem->item_type_id;
				$woidArr->unit_type_id  					= $unit_type_id;
				$woidArr->item_id  							= $soItem->item_id;
				$woidArr->grey_quality  					= $soItem->grey_quality;
				$woidArr->dyeing_color  					= $soItem->dyeing_color;
				$woidArr->coated_pvc  						= $soItem->coated_pvc;
				$woidArr->extra_job  						= $soItem->extra_job;
				$woidArr->print_job  						= $soItem->print_job;
				$woidArr->expect_delivery_date  			= $soItem->expect_delivery_date;
				$woidArr->order_item_priority  				= $soItem->order_item_priority;
				$woidArr->pcs  								= $soItem->pcs;
				$woidArr->cut  								= $soItem->cut;
				$woidArr->meter  							= $soItem->meter;
				$woidArr->status  							= 1;
...
/*
	...
	*/
	...
	{ 	
		...
		return view($viewName, compact("data", "dataI", "dataIC", "dataIYR", "itemId", "itemTypeId", "workOrderId", "dataWI"));
	}
	...
	public function create_work_order_for_weaving(Request $request)
    {
        ...
        $dataOrder 	= WorkOrder::where('work_order_id', $workOrderId)->with('WorkOrderItemDetail')->first();
        $dataPT 	= ProcessItem::where('id', '>', $dataOrder->process_type_id)->first();
        $lastPsnl 	= $dataPT->process_sl_no_last + 1;
        $dataPr 	= ProcessRequirement::where('process_type_id', $dataPT->id)->where('status', '1')->first();
        $itemTypeId = $dataPr->item_type_id;
        ...
        if ($workOrderId) 
		{
            ...
            if ($neworkOrderId) 
			{
                foreach ($dataOrder->WorkOrderItemDetail as $soItem) 
				{
                    $customerId 	= SaleOrder::where('sale_order_id', $soItem->sale_order_id)->value('individual_id');
                    $unit_type_id 	= Item::where('item_id', $soItem->item_id)->value('unit_type_id');
					$woidArr = new WorkOrderItemDetail;
