...
class MasterController extends Controller
{
    ...
    { 
		...
		$dataP = Master::where('status', '=', '1')->paginate(20); 
		// echo "<pre>";  print_r($dataI); exit; 
		return view('html.master.show-masters',compact("dataP","qsearch","dataI"));  
    }
    ...
    public function edit_master ($id)
	{
        ...
        $dataI = Individual::where('id', $indID)->first(); 	        
		return view('html.master.edit-master',compact("data","dataI"));
	}

    public function create_master($id)
    {
		$indID = base64_decode($id);
		$data = Individual::where('id', $indID)->first(); 		 
		return view('html.master.add-master',compact("data","indID"));
    }
	...
	public function store_master(Request $request )
    {
	  ...
	  // echo "<pre>"; print_r($request->all()); exit;
		...
		$obj->master_from  		    = $request->master_from;
		$obj->master_to  		    = $request->master_to;
		...
		if($is_saved)
		{
            ...
            return redirect("/show-masters/".base64_encode($obj->individual_id));
		}
	...
	//  echo "<pre>"; print_r($request->all()); exit;
		...
		$obj = Master::where('id','=', $request->id)->first();
	...
	//	$obj->name  				= $request->name;
		...
		if($is_saved)
		{
			...
			Session::put("messageClass","successClass");
			return redirect("/show-masters/".base64_encode($request->individual_id));
		}
