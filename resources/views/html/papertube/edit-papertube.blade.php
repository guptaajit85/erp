		...
		{!! CommonController::display_message('message') !!}
          <div class="panel panel-bd lobidrag">
            ...
            <div class="panel-body">
              <form class="col-sm-6" role="form" name="domainEdit" id="domainEdit" method="post" action="{{ url('/update_papertube') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                  <label> Name   <span style="color:#ff0000;">*</span></label>
                  <input type="text" class="form-control" name="paperTube_name" value="{{ htmlentities($data->paperTube_name) }}" id="paperTube_name" placeholder="Enter Paper Tube Name" required>
                  <input type="hidden" class="form-control" name="id" id="id" value="{{ htmlentities($data->id) }}"  >
                </div>
                <div class="form-group">
                  <label> Features  <span style="color:#ff0000;">*</span> </label>
                  <input type="text" class="form-control" name="features" value="{{ htmlentities($data->features) }}" id="features" placeholder="Enter Features" required>
                </div>
