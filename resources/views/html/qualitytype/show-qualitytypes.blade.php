    ...
    <div class="panel panel-bd lobidrag">
      ...
      <div class="panel-body">
        ...
        <div class="row" style="margin-bottom:5px">
          <form action="{{ route('show-qualitytypes') }}" method="GET" role="search">
            @csrf
          ...
          <div class="col-sm-2 col-xs-12"> <a class="btn btn-add" href="add-qualitytype"> <i class="fa fa-plus"></i> Add
            Quality Type </a> </div>
        </div>
        <!-- Plugin content:powerpoint,txt,pdf,png,word,xl -->
        <div class="table-responsive">
          <table id="dataTableExample1"
            ...
            <tr id="Mid{{ $data->id }}">
              <td> {{ $data->qualitytype_name }} </td>
              <td><a href="edit-qualitytype/{{ base64_encode($data->id) }}"
                                                            class="tooltip-info"><i class="fa fa-pencil"></i></a></td>
              <td class="center"><a href="javascript:void(0);"
                                                            onclick="deleteQualityType({{ $data->id }})"
...
var siteUrl = "{{url('/')}}";
        function deleteQualityType(id) {
            if (confirm("Do you realy want to delete this record?")) {
                jQuery.ajax({
                    type: "GET",
                    url: siteUrl + '/' +"ajax_script/deleteQualityType",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "FId": id,
                    },
