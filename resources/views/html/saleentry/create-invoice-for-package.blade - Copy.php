                  ...
                  <tbody>
                    <tr>
                      ...
                      <td style="width:300px;"><label for="exampleInputCategory1">Customer Name <span style="color:#ff0000;">*</span></label>
                        ...
                        <label for="exampleInputCategory1">Email :
                        <?=$dataPO['Individual']->email;?>
                        </label>
                        </br>
                        <input type="hidden" name="email" id="email" class="form-control" placeholder="Email" value="<?=$dataPO['Individual']->email;?>" >
                        <label for="exampleInputCategory1">GSTIN :
                        <?=$dataPO['Individual']->gstin;?>
                        </label>
                        <input type="hidden" name="cst" id="cst" class="form-control" placeholder="GSTIN" value="<?=$dataPO['Individual']->gstin;?>">
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
					<?php 
						$totNetAmount = 0;
						$i = 1;
						foreach($dataPO['packagingOrderItems'] as $rowArr) {
                  ...
                  <tr id="tr_<?=$i;?>">
                    <td><?=$i;?></td>
                    <td><input value="6" type="hidden" name="item_id_arr[]" readonly="" value="<?=$rowArr->item_id;?>">
                    ...
                    <td><input readonly="" value="<?=$rowArr->amount;?>" type="text" class="form-control" name="saleprice_wot_arr[]"></td>
                    ...
                    <td><input readonly="" value="<?=round($rowArr->amount-$rowArr->discount_amount);?>" type="text" class="form-control" name="saleprice_arr[]">
                    </td>
					...
					<td><input readonly="" value="<?=$igstAmount;?>" type="text" class="form-control" name="total_arr[]"></td>
                    <td><input readonly="" value="<?=($saleprice+$igstAmount);?>" type="text" class="form-control" name="amount_arr[]">
                    </td>
                  </tr>
                  <?php $i++;} ?>
                </tbody>
...
<script>
  function calculateSalePrice(index) {
	...
	// Update saleprice_wot_arr[] field
	document.getElementsByName("saleprice_wot_arr[]")[index].value = salePriceWithoutTax.toFixed(2);
	...
	// Update saleprice_arr[] field
	document.getElementsByName("saleprice_arr[]")[index].value = salePriceAfterDiscount.toFixed(2);
    // Calculate IGST amount
    var igstAmount = (igstPercentage / 100) * salePriceAfterDiscount;

    // Update total_arr[] field
    document.getElementsByName("total_arr[]")[index].value = igstAmount.toFixed(2);
    ...
    // Update amount_arr[] field
    document.getElementsByName("amount_arr[]")[index].value = finalAmount.toFixed(2);

    // Update subtotal field by summing up all amount_arr[] values
    updateSubtotal();
  }

  // Function to update subtotal, total, and other related fields
  function updateSubtotal() {
    var amountFields = document.getElementsByName("amount_arr[]");
    var subtotal = 0;
